...
int main()
{
    ...
    for (const auto& input_file : input_files)
    {
        ...
        int max_haters = 0, max_fans = 0;
        for (const auto& [ingredient, fans] : data.ingr_to_fans)
            if (fans.size() > max_fans)
                max_fans = (int)fans.size();

        for (const auto& [ingredient, haters] : data.ingr_to_fans)
            if (haters.size() > max_haters)
                max_haters = (int)haters.size();

        std::cout << "max_fans = " << max_fans << ", max_haters = " << max_haters << std::endl;
    }
    return 0;
}